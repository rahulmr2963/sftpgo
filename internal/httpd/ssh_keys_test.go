...
func TestSSHKeyGenerationIntegration(t *testing.T) {
	...
	if deserializedKeyPair.GeneratedAt.IsZero() {
		t.Error("GeneratedAt timestamp missing after JSON roundtrip")
	}
	...
	if !strings.Contains(jsonString, "generated_at") {
		t.Error("JSON output missing 'generated_at' field")
	}
}
